...
> **Superseded (2026-09) — every feature exists at every level of detail.**
> The owner plans ravines and large deep caves, and asked that rendering not
> need rewriting when they arrive. With caves at level 0 only, any such feature
> would vanish at a distance, and each would need its own special case.
>
...
> (four columns averaged, rounded down). The single-point sampling this section
> rejected was the problem, not caves at a distance: measured against the blocks
> each cell stands for, one corner sample is wrong for up to 2.0% of clearly-decided
> cells, the majority for 0.00% (terrain at most 0.03% at cells of 4 and 8,
> 0.00% at 2). `a_coarse_cell_is_what_most_of_its_blocks_are` holds any future
> feature to that, per region.
>
> It is affordable because of what landed around it: covered border faces left
> out, and visibility culling that never generates what no line of sight reaches
> (`crates/world/src/visibility.rs`). A player's view at the surface draws 1.25M
> triangles with caves everywhere; the bench's orbit, which looks at the region
> from outside through its cut edges, 1.99M (`BENCHMARKS.md` ⁵¹).
>
