...
| **2.12b** | **The game window joins a server** — the correction channel (`SelfState`, input sequence numbers, registry fingerprints), then `Game` over a `Link` and `--connect` | Added 2026-09-05, owner’s call. Block 2.12 made two *processes* talk; it did not make two *people* able to play, because the GUI had no way to join one. The gate criterion said “a real socket, two processes” and passed while that was still true — a criterion that can be met without the goal. |
